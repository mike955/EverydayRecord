...
import StudyPlanModel from '../models/study_plan';
		...
		if(!req.session.user){
			...
			if(study_plan_data.subject === ''){
				...
				******************************/
				...
				new_data.cause = '';
				//查找用户id
				let uid = await StudyPlanModel.findByName(req.session.user.username);

				if(uid[0].id){
					new_data.accountID = uid[0].id;
					let result = await StudyPlanModel.create(new_data);
