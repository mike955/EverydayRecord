...
exports.History = async function(req, res){
	...
	try{	//查找用户id
		...
		if(uid[0].id){//查找用户的账单数据
			result = await Bill.findAll(uid[0].id);
...
exports.statistic = async function(req, res){
	...
	*************************************************/
	...
	try{
		uid = await Bill.findByName(username);
		if(uid[0].id){
			sumMoney = await Bill.sumMoney(uid[0].id);
			sumWays[0] = await Bill.sumWays(1, uid[0].id);
			sumWays[1] = await Bill.sumWays(2, uid[0].id);
			sumWays[2] = await Bill.sumWays(3, uid[0].id);
			sumWays[3] = await Bill.sumWays(4, uid[0].id);
			sumWays[4] = await Bill.sumWays(5, uid[0].id);
			sumWays[5] = await Bill.sumWays(6, uid[0].id);
			sumWays[6] = await Bill.sumWays(7, uid[0].id);
			sumWays[7] = await Bill.sumWays(8, uid[0].id);
			sumWays[8] = await Bill.sumWays(9, uid[0].id);

			sumTypes[0] = await Bill.sumType(1, uid[0].id);
			sumTypes[1] = await Bill.sumType(2, uid[0].id);
			sumTypes[2] = await Bill.sumType(3, uid[0].id);
			sumTypes[3] = await Bill.sumType(4, uid[0].id);
			sumTypes[4] = await Bill.sumType(5, uid[0].id);
			sumTypes[5] = await Bill.sumType(6, uid[0].id);
			sumTypes[6] = await Bill.sumType(7, uid[0].id);
			sumTypes[7] = await Bill.sumType(8, uid[0].id);
		}else{
			res.render('user/signin', {
			});
		}
